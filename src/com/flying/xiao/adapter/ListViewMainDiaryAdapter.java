...
public class ListViewMainDiaryAdapter extends BaseAdapter {
	 ...
	 * @param pubcommentLin
	 */
	public ListViewMainDiaryAdapter(Context context, View pubcommentLin,
			List<XContent> data, int resource) {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			listItemView.userface = (ImageView) convertView
					.findViewById(R.id.diary_listitem_userface);
			listItemView.username = (TextView) convertView
					.findViewById(R.id.diary_listitem_username);
			listItemView.content = (TextView) convertView
					.findViewById(R.id.diary_listitem_content);
			listItemView.gridView = (GridView) convertView
					.findViewById(R.id.diary_listitem_grid);
			listItemView.date = (TextView) convertView
					.findViewById(R.id.diary_listitem_date);
			listItemView.btncomment = (TextView) convertView
					.findViewById(R.id.diary_listitem_comment);
			listItemView.relies = (LinearLayout) convertView
					.findViewById(R.id.diary_listitem_relies);
			listItemView.praiseLin = (LinearLayout) convertView
					.findViewById(R.id.diary_listitem_praise_lin);
			listItemView.praiseImage = (ImageView) convertView
					.findViewById(R.id.diary_listitem_praise_image);
			listItemView.praiseText = (TextView) convertView
					.findViewById(R.id.diary_listitem_praise);
			// 设置控件集到convertView
		...
		listItemView.date.setText(StringUtils.friendly_time(con.getConPubTime()
				.toString()));
		String faceURL = con.getConImageUrl();
		if (faceURL.endsWith("portrait.gif") || StringUtils.isEmpty(faceURL)) {
			listItemView.userface.setImageResource(R.drawable.widget_dface);
		} else {
			ImageManager2.from(context).displayImage(listItemView.userface,
		...
		if (xpraises != null && xpraises.size() > 0) {
			...
			for (int i = 0; i < len; i++) {
				...
				// if (ac.isLogin() &&
				// xp.getUserInfo().getId().equals(ac.getUserInfo().getId()))
				// {
		...
		if (con.isMeIsPraise()) {
			...
			listItemView.praiseImage
					.setImageResource(R.drawable.praise_already);
		} else {
				...
				if (ac.isLogin()) {
					...
					if (con.isMeIsPraise()) { // 如果已经赞了
						for (XPraise xp : xpraises) {
							if (ac.isLogin()
									&& xp.getUserInfo().getId()
											.equals(ac.getUserInfo().getId())) {
								praise.copy(xp);
								con.getPraiseList().remove(xp);
								break;
							}
		...
		if (listComments != null && listComments.size() > 0) {
			...
			for (final XComment reply : listComments) {
				View view2 = listContainer
						.inflate(R.layout.comment_reply, null);
				TextView tv2 = (TextView) view2
						.findViewById(R.id.comment_reply_content);
				// String text = "";
