...
public class NetControl {
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					XUserInfo user = HttpUtil.login(appContext, userName,
							password);
	 ...
	 * @param handler
	 */
	public void alertPassword(final String oldPassword,
			final String newPassword, final String userId, final Handler handler) {
		new Thread() {
			@Override
			public void run() {
				Message msg = new Message();
				try {
					...
					Base base = HttpUtil.alertPassword(appContext, oldPassword,
							newPassword, userId);
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					if (user == null || user.getErrorCode() != 0) {
						if (user.getErrorCode() == Constant.ErrorCode.USER_REGISTER_USERNAME_HAVE) { // 用户名已存在
	...
	public void getContentData(final int type, final long userId,
		...
		new Thread() {
			...
			public void run() {
				Message msg = new Message();
				List<XContent> xConList = HttpUtil.getContentData(appContext,
						type, userId, page, isMyCollect);
	 ...
	 * @param handler
	 */
	public void getContentData(final int type, final int page,
			final Handler handler) {
		getContentData(type, -1, page, false, handler);
	}

	public void getMarketData(final int marketType, final long userId,
			final int page, final Handler handler) {
		new Thread() {
			@Override
			public void run() {
				Message msg = new Message();
	...
	public void getWzData(final int wzType, final long userId, final int page,
			final Handler handler) {
		new Thread() {
			@Override
			public void run() {
				...
				List<XContent> xConList = HttpUtil.getWzData(appContext,
						Constant.ContentType.CONTENT_TYPE_NEWS, wzType, userId,
						page);
				if (xConList == null || xConList.size() == 0) {
					msg.what = Constant.HandlerMessageCode.DEPARTMENT_DETAIL_LOAD_DATA_FAIL;
					msg.obj = "获取信息出错...";
	...
	public void getContentDetail(final int contentType, long contentId,
		...
		final String url = URLs.URL_GET_CONTENT_DETAIL + "?type=" + contentType
				+ "&id=" + contentId;
		new Thread() {
			@Override
			public void run() {
	 ...
	 *            ui接收消息
	 */
	public void pubComment(final long userId, final long contentId,
			final String commentInfo, final long replyId, final Handler handler) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					XComment base = HttpUtil.pubConmment(appContext, userId,
							contentId, commentInfo, replyId);
	 ...
	 * @param handler
	 */
	public void pubMessage(final long userId, final String messageInfo,
			final int messageReplyId, final int messageMainId,
			final Handler handler) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					XMessage base = HttpUtil.pubMessage(appContext, userId,
							messageInfo, messageReplyId, messageMainId);
					if (base.getErrorCode() != 0) {
						if (base.getErrorCode() == Constant.ErrorCode.USER_NOT_LOGIN) {
							msg.what = Constant.HandlerMessageCode.USER_NOT_LOGIN;
						} else {
	...
	public void praiseOpreate(final XPraise xp, final boolean isCancel,
			final Handler handler) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					XPraise base = HttpUtil.praiseContent(appContext, userId,
							contentId, isCancel);
	...
	public void collectOperate(final long contentid, final boolean isCancel,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					Base base = HttpUtil.collectionOperate(appContext,
							contentid, isCancel);
					if (base == null || base.getErrorCode() != 0) {
						if (base.getErrorCode() == Constant.ErrorCode.USER_NOT_LOGIN) {
							msg.what = Constant.HandlerMessageCode.USER_NOT_LOGIN;
	...
	public void getComments(final long contentid, final int page) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					List<XComment> commentlist = HttpUtil.getComments(
							appContext, contentid, page);
					if (commentlist == null) {
	...
	public void getUserInfos(final int typeid, final int page,
			final Handler handler) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					List<XUserInfo> userInfos = HttpUtil.getUserInfos(
							appContext, typeid, page);
					if (userInfos == null) {
	...
	public void getMyDynamic(final Handler handler, final int page) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					List<XDynamic> dynamics = HttpUtil.getMyDynamic(appContext,
							page);
					if (dynamics == null) {
	...
	public void getMyMessage(final Handler handler, final int page) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					List<XMessage> dynamics = HttpUtil.getMyMessage(appContext,
							page);
					if (dynamics == null) {
	...
	public void changeUserInfo(final String userJson,
			final String headImageFile, final Handler handler) {
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					Base base = HttpUtil.changeUserInfo(appContext, userJson,
							headImageFile);
					if (base.getErrorCode() != 0) {
						if (base.getErrorCode() == Constant.ErrorCode.USER_NOT_LOGIN) {
							msg.what = Constant.HandlerMessageCode.USER_NOT_LOGIN;
	...
	public void pubDiaryOrLost(final String content, final List<String> files,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				Message msg = new Message();
				try {
					Base base = HttpUtil.pubDiaryOrLost(appContext, content,
							files, type);
					if (base == null || base.getErrorCode() != 0) {
						if (base.getErrorCode() == Constant.ErrorCode.USER_NOT_LOGIN) {
							msg.what = Constant.HandlerMessageCode.USER_NOT_LOGIN;
