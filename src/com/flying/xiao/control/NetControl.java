...
public class NetControl
{
	...
	public void getContentData(final int type,final long userId,final int page,final boolean isMyCollect,final Handler handler){
		new Thread(){
			@Override
			public void run() {
				Message msg =new Message();
				List<XContent> xConList =HttpUtil.getContentData(appContext, type,userId, page,isMyCollect);
	...
	public void getMarketData(final int marketType,final long userId,final int page,final Handler handler){
		new Thread(){
			...
			public void run() {
				Message msg =new Message();
				List<XContent> xConList =HttpUtil.getMarketData(appContext, Constant.ContentType.CONTENT_TYPE_MARKET,userId, page);
				if(xConList==null||xConList.size()==0){
	...
	public void getWzData(final int wzType,final long userId,final int page,final Handler handler){
		new Thread(){
			...
			public void run() {
				Message msg =new Message();
				List<XContent> xConList =HttpUtil.getWzData(appContext, Constant.ContentType.CONTENT_TYPE_NEWS, wzType,userId, page);
				if(xConList==null||xConList.size()==0){
					msg.what=Constant.HandlerMessageCode.DEPARTMENT_DETAIL_LOAD_DATA_FAIL;
					msg.obj = "获取信息出错...";
				}
...
//	}
	 ...
	 * @param handler
	 */
	public void pubMessage(final long userId,final String messageInfo,final int messageReplyId,final int messageMainId,final Handler handler){
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				try
				{
					XMessage base=HttpUtil.pubMessage(appContext, userId, messageInfo,messageReplyId,messageMainId);
					if(base.getErrorCode()!=0)
					{
					if(base.getErrorCode()==Constant.ErrorCode.USER_NOT_LOGIN){
						msg.what=Constant.HandlerMessageCode.USER_NOT_LOGIN;
	...
	public void getMyFriends(final Handler handler){
		new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				try
				{
					List<XUserInfo> userInfos=HttpUtil.getMyFriends(appContext);
					if(userInfos==null){
	...
	public void pubDiaryOrLost(final String content,final List<String> files,final Handler handler,final int type ){
		new Thread(new Runnable()
		{
